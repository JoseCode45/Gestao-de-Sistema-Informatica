...
const Armazem = () => {
  ...
  const toggleEstado = async (id, currentEstado) => {
    ...
    try {
      if (novoEstado === 'inativo') {
        await axios.delete(`${BASE_URL}/armazem/${id}`, {
          data: { alteradorID },
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        ...
        await axios.patch(`${BASE_URL}/armazem/${id}`, { alteradorID }, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
        });
  ...
  const filteredData = data.filter((armazem) =>
    armazem.Morada?.toLowerCase().includes(search.toLowerCase()) ||
    armazem.AreaM2?.toLowerCase().includes(search.toLowerCase()) ||
    armazem.CriadorNome?.toLowerCase().includes(search.toLowerCase()) ||
    armazem.AlteradorNome?.toLowerCase().includes(search.toLowerCase()) ||
    armazem.ID.toString().includes(search)
  );
        ...
        <table className="element table table-responsive table-hover table-striped ">
          <thead>
            <tr>
              ...
              <th>Atualizado em</th>
              <th>Estado</th>
              <th> <Link to="/admin/armazem/criar" className='btn btn-outline-primary button-sucess w-40 rounded-0'>NOVO</Link></th>
            </tr>
          </thead>
          <tbody className="element ">
            {currentData.map((armazem) => (
              <tr key={armazem.ID}>
                <td><strong>{armazem.ID}</strong></td>
                <td>{armazem.Morada}</td>
                <td>{armazem.AreaM2}</td>
                <td>{armazem.CriadorID} {armazem.CriadorNome || "N/A"}</td>
                <td>{armazem.AlteradorID} {armazem.AlteradorNome || "N/A"}</td>
                <td>{new Date(armazem.CreateTime).toLocaleString()}</td>
                <td>{new Date(armazem.UpdateTime).toLocaleString()}</td>
                <td className="text-center align-middle">
                  <button
                    className={`btn btn-sm ${armazem.Estado === 'ativo' ? 'btn-success' : 'btn-danger'}`}
                    onClick={() => toggleEstado(armazem.ID, armazem.Estado)}
                    style={{ width: '90px' }}
                  >
                    {armazem.Estado}
                  </button>
                </td>
                <td className="text-center align-middle" > <Link to={`/admin/armazem/edit/${armazem.ID}`} className='btn btn-outline-dark button-sucess w-40 rounded-0'>EDIT</Link> </td>
              </tr>
            ))}
          </tbody>
        </table>
