...
import './style.css'; // manter padrão, mesmo que vazio

const PromocaoProduto = ({ associadas, disponiveis, setAssociadas }) => {
  ...
  const handleAdicionar = () => {
    ...
    if (produtoObj && !associadas.some(p => p.ID === produtoObj.ID)) {
      ...
      setSelectedProduto('');
      console.log('Produtos associados à promoção:', associadas);
    }
  };
      ...
      <table className="table table-bordered selec" style={{ borderColor: '#dee2e6' }}>
        ...
        <tbody>
          {associadas.length > 0 ? (
            ...
            <tr>
              <td colSpan="2">Nenhum produto associado à promoção</td>
            </tr>
          )}
        </tbody>
      </table>
...
export default PromocaoProduto;
