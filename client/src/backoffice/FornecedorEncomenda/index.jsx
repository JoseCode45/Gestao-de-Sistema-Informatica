...
const FornecedorEncomenda = () => {
  ...
  useEffect(() => {
    axios.get(`${BASE_URL}/fornecedor-encomenda`, {
      headers: {
  ...
  const filteredData = data.filter((item) =>
    ...
    item.FornecedorNome?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <>
      <div className="fixd d-flex justify-content-between align-items-center mb-3">
        <h1>Encomendas de Fornecedores</h1>
        ...
        <table className="element table table-responsive table-hover table-striped">
          <thead>
            <tr>
              ...
              <th>Estado</th>
              <th><Link to={`/admin/fornecedorencomenda/criar`} className='btn btn-outline-dark w-40 rounded-0'>NOVO</Link></th>
            </tr>
          </thead>
          <tbody>
            {currentData.map((item) => (
              <tr key={item.ID}>
                ...
                <td>{item.FornecedorNome || 'N/A'}</td>
                <td>{new Date(item.DataPedido).toLocaleString()}</td>
                <td>{item.DataEntrega ? new Date(item.DataEntrega).toLocaleString() : '—'}</td>
                <td>{Number(item.TotalEncomenda).toFixed(2)} €</td>
                <td>{(item.TotalEncomenda * 0.13).toFixed(2)} €</td>
                ...
                <td className="text-center align-middle">
                  <Link to={`/admin/fornecedorencomenda/edit/${item.ID}`} className='btn btn-outline-dark w-40 rounded-0'>EDIT</Link>
                </td>
              </tr>
            ))}
          </tbody>
...
export default FornecedorEncomenda;
