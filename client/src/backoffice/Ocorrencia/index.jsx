...
const Ocorrencia = () => {
  ...
  const toggleEstado = async (id, estadoID) => {
    const novoEstadoID = estadoID === 1 ? 2 : 1; // Ex: 1 = pendente, 2 = resolvido

    try {
      await axios.patch(`${BASE_URL}/ocorrencia/${id}`, { alteradorID, EstadoID: novoEstadoID }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      setData(prevData =>
        prevData.map(item =>
          item.ID === id ? { ...item, EstadoID: novoEstadoID } : item
        )
      );
    } catch (err) {
  ...
  const analisarOcorrencia = async (id, estadoID) => {
    if (estadoID !== 1) return; // Só muda o estado se estiver em "Pendente"

    try {
      await axios.patch(`${BASE_URL}/ocorrencia/${id}/analisar`, {}, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
      console.log(`Ocorrência ${id} marcada como Em Análise`);
    } catch (error) {
              ...
              <tr key={oc.ID}>
                ...
                <td className="text-center align-middle">
                  <button
                    ...
                    onClick={async () => {
                      await analisarOcorrencia(oc.ID, oc.EstadoID);
                      navigate(`/admin/ocorrencia/view/${oc.ID}`);
                    }}
                  >
                    Visualizar
                  </button>
